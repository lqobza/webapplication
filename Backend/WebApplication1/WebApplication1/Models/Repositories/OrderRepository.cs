    ...
    public void InsertOrderItem(SqlConnection connection, SqlTransaction transaction, int orderId, OrderItemDto item)
    {
        ...
        using (var command = new SqlCommand(query, connection, transaction))
        {
            ...
            command.Parameters.AddWithValue("@price",
                item.Price); //TODO: price should be queried (only during insert in case price changes later the customer should still get the order for the price that was displayed during the purchase) and calculated on backend site based on the ordered items
            //in theory this should be working like this, test it
